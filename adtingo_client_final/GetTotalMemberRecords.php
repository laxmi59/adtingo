...
if($zipcodes!="" && $zipcodedist!="")
{
	...
	if(count($zipcodedb)>0 && $zipcodedb!='undefined')
	{
		...
		$where.="  and tbl_members.zipcode IN ($zipcodes)";
}
if($zipcodes!="" && $zipcodedist ==0)
{
	if(count($zipcodes)>0 && $zipcodes!='undefined')
	{
...
//echo sizeof($GetTotalSearchListRecords);
//echo $TotalSearchRecordslist
//echo sizeof($GetTotalSearchListRecords); exit;

   $SqlSearch_result="select  distinct tbl_members.memberid,contact_time from tbl_members left join tbl_listedmembers on  tbl_members.memberid=tbl_listedmembers.memberid  ,tbl_member_metropolitian  where $where_1 and tbl_members.memberid = tbl_member_metropolitian.memberid and tbl_listedmembers.memberid is NULL"; 
  //echo $SqlSearch_result;exit;   
$ResSearch_result=$object->ExecuteQuery($SqlSearch_result);
//echo  $TotalSearchRecords=$object->NumRows($ResSearch_result); exit;
while($ResSearch_resultRec=$object->FetchArray($ResSearch_result))
{
