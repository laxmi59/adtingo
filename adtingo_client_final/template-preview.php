...
if($_REQUEST['cid']!="")
{
	...
	$email_to_frd_link='<img src="http://clients.adtingo.com/Campaign_templates/Campaign_template1/images/content-buttonemailtofriend.jpg" alt="" border="0" width="90" height="30">';	
	...
	//$localdealdesc = substr($localdealdesc1, 0, strpos($localdealdesc1, ' ', 300)); 
	  ...
	  $listingid=$RecCampaign_info['listing_id'];
		$publisher='0905768917';
		$reference_id=$RecCampaign_info['ref_id'];
		$placement='home';
	$memberid="";	
...
<script type='text/javascript' src='http://images.citysearch.net/assets/pfp/scripts/tracker.js'></script><noscript>
	...
	if($GetTemplateinfoRec['paragraph1']!="")
		$listofthinks_to_come1 = "<li style='padding-bottom:5px;'>".stripslashes($GetTemplateinfoRec['paragraph1'])."</li>";
	if($GetTemplateinfoRec['paragraph2']!="")	    
		$listofthinks_to_come2 = "<li style='padding-bottom:5px;'>".stripslashes($GetTemplateinfoRec['paragraph2'])."</li>";	
	if($GetTemplateinfoRec['paragraph3']!="")
		$listofthinks_to_come3 = "<li style='padding-bottom:5px;'>".stripslashes($GetTemplateinfoRec['paragraph3'])."</li>";
	if($GetTemplateinfoRec['paragraph4']!="")	
		$listofthinks_to_come4 = "<li style='padding-bottom:5px;'>".stripslashes($GetTemplateinfoRec['paragraph4'])."</li>";
	if($GetTemplateinfoRec['paragraph5']!="")	
		$listofthinks_to_come5 = "<li style='padding-bottom:5px;'>".stripslashes($GetTemplateinfoRec['paragraph5'])."</li>";
	if($GetTemplateinfoRec['paragraph6']!="")		    
		$listofthinks_to_come6 = "<li style='padding-bottom:5px;'>".stripslashes($GetTemplateinfoRec['paragraph6'])."</li>";
	...
	$mailMessage = str_replace("#CAMPAIGNID#", "$CampaignID", "$mailMessage");
...
if(isset($_POST['steppreview_x_x'])){
	...
	if($ClientFetch['CCTypeID']=='' || $ClientFetch['CCNo']=='' || $ClientFetch['CCExpMon']=='' || $ClientFetch['CCExpYear']=='' || 	$ClientFetch['CCName']=='' || $ClientFetch['CCVNo']==''){
		header("location:payment-gateway.php?cid=".base64_encode($CampaignID)."");
		exit;
	}else{		
...
<title><?php echo SITETITLE."Campaign Preview"?></title>
