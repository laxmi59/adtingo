...
$GetAllCampaignRecordsQry="select * from tbl_campaigns where schedule_date = approved_date  and status=2";  
...
$campainid=base64_encode($GetAllCampaignRecordsRec['campaign_id']);
	...
	try{	
		...
		$citygridlink= $GetAllCampaignRecordsRec['destination_url'];
...
<img src="http://clients.adtingo.com/Campaign_templates/Campaign_template1/images/content-buttonfbshare.jpg" alt="" border="0" width="95" height="30"></a>';	
		...
		$email_to_frd_link='<a href="http://adtingo.com/email-to-frd?cid='.base64_encode($GetAllCampaignRecordsRec['campaign_id']).'&mid=[-EMAILADDR-]"><img src="http://clients.adtingo.com/Campaign_templates/Campaign_template1/images/content-buttonemailtofriend.jpg" alt="" border="0" width="90" height="30"></a>';	
			
		$today = date("F j, Y");
		
		
		
		$listingid=$GetAllCampaignRecordsRec['listing_id'];
		$publisher='0905768917';
		$reference_id=$GetAllCampaignRecordsRec['ref_id'];
		$placement='home';
...
<script type='text/javascript' src='http://images.citysearch.net/assets/pfp/scripts/tracker.js'></script><noscript>
    <img src='http://api.citysearch.com/tracker/imp?action_target=listing_profile&listing_id=$listingid&publisher=$publisher&reference_id=$reference_id&
    placement=$placement' width='1' height='1' alt='' />
		...
		$mailMessage = str_replace("#DESTINATIONURL#","$othertempurl","$mailMessage");
		...
		$mailMessage = str_replace("#CAMPAIGNID#", "$campainid", "$mailMessage");
		//javascript variables
		$mailMessage = str_replace("#LISTINGID#", "$listingid", "$mailMessage");
		$mailMessage = str_replace("#PUBLISHER#", "$publisher", "$mailMessage");
		$mailMessage = str_replace("#REFERENCEID#", "$reference_id", "$mailMessage");
		$mailMessage = str_replace("#PLACEMENT#", "$placement", "$mailMessage");
		$mailMessage = str_replace("#SCRIPT#", "$js", "$mailMessage");
		$mailMessage = str_replace("#CITYGRIDLINK#", "$citygridlink", "$mailMessage");
		...
		$k=0;
		if(sizeof($member_details) >1000){
		for($i=1000;$i<sizeof($member_details);$i=$i){
			$slice1 = array_slice($member_details, $k, $i);
			//echo $member_details."---".$k."---".$i."---".sizeof($slice1)."<br>";
			$smft_res =  $soapClient->__call("sendMessagesFromTemplate", array($cst_res, $slice1));
			$slice1 = array_splice($member_details, $k, $i);
		}  
		}else{
...
<img src="http://clients.adtingo.com/Campaign_templates/Campaign_template1/images/content-buttonfbshare.jpg" alt="" border="0" width="95" height="30"></a>';	
		...
		$citygridlink1=$GetAllCampaignRecordsRec['destination_url'];
		...
		$wpmailMessage = str_replace("#DESTINATIONURL#", "$othertempurl", "$wpmailMessage");
...
<script type='text/javascript' src='http://images.citysearch.net/assets/pfp/scripts/tracker.js'></script><noscript>
    ...
    placement=$placement' width='1' height='1' alt='' />
		...
		//javascript variables
		$wpmailMessage = str_replace("#LISTINGID#", "$listingid", "$wpmailMessage");
		$wpmailMessage = str_replace("#PUBLISHER#", "$publisher", "$wpmailMessage");
		$wpmailMessage = str_replace("#REFERENCEID#", "$reference_id", "$wpmailMessage");
		$wpmailMessage = str_replace("#PLACEMENT#", "$placement", "$wpmailMessage");
		$wpmailMessage = str_replace("#SCRIPT#", "$js", "$wpmailMessage");
		$wpmailMessage = str_replace("#CITYGRIDLINK#", "$citygridlink1", "$wpmailMessage");
		
		//echo $wpmailMessage ;exit;
		//$postdesinationurl = $GetAllCampaignRecordsRec['destination_url']."?listing_id=".$listingid."&client_ip=184.73.167.196&reference_id=".$reference_id."&publisher=".$publisher."&placement=".$placement."&api_key=gkpyqcng3yj9w4zn9ns2rcr3&format=xml";
		$postdesinationurl = $GetAllCampaignRecordsRec['destination_url']."?listing_id=".$listingid."&client_ip=184.73.167.196&reference_id=".$reference_id."&publisher=".$publisher."&placement=".$placement."&format=xml";
		...
		//$post_name4 = mt_rand(5, 10);
		//post_name5 =$post_name3.$post_name4;
