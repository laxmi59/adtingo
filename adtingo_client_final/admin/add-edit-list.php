...
if(isset($_REQUEST['button3']) || $_REQUEST['button3t']!='')
{	
extract($_POST);
	...
	{	
		...
		if($cols==0){
			$insertcatInfo=sprintf("insert into tbl_metropolitian_list(area_name,status,state,lat,lng) values('%s',%d,'%s','%s','%s')",$object->stripper($_POST['areaname']),'1',$object->stripper($_POST['state']),$object->stripper($_POST['main1']),$object->stripper($_POST['main2'])); 
...
else
{
	...
	$cols=$object->NumRows($errchkext);
	if($cols==1){
		 $updatecatinfo=sprintf("update tbl_metropolitian_list set  area_name='%s', state='%s', lat='%s', lng='%s' where  area_id=%d",$object->stripper($_POST['areaname']),$object->stripper($_POST['state']),$object->stripper($_POST['lat']),$object->stripper($_POST['lng']),$object->stripper($_REQUEST['area_id'])); 
...
function lat_lng()
{
	var d2=document.frmchart.areaname.value;
	var d3=document.frmchart.state.value;
	var addr=d2+','+d3;
	//alert(addr);
	showAddress(addr);
}
function showAddress(address) 
{
	...
	//alert(address);
		...
		function(point) {
			...
			//alert(msg);
			document.frmchart.main1.value=point.lat();
			document.frmchart.main2.value=point.lng();
			document.frmchart.button3t.value="submit";
...
$option_rule=$displaycatrewrite['option_value'];
			...
			<input type="hidden" name="option_rule" value="<?php echo $option_rule?>" />			</td>
		  </tr>
		  
		  
		  <tr>
		    <td height="30" align="left"><strong>State</strong></td>
		    <td align="left">
			<select name="state">
			<option>Select State</option>
			<?php 
			$getStates="select * from tbl_state";
			$QryGetStates=$object->ExecuteQuery($getStates);
			while($FetGetStates=$object->FetchArray($QryGetStates)){?>
				<option value="<?php echo $FetGetStates['state_abr']?>" <?php if($QryGetareaDetailsRec['state']==$FetGetStates['state_abr']){ echo "selected";}?>><?php echo $FetGetStates['state']?></option>
			<?php }
			?>
</select>
<input type="hidden" id="stateerr" name="stateerr">

			</td>
		    </tr>
		  <tr>
			<td height="30" align="left">&nbsp;</td>
			<td align="left"><input type="hidden" name="button3t" id="button3t" />
			  <input name="button3" type="button" class="button" id="button3" value="Submit"  onclick="lat_lng();" />								  </td>
		  </tr>
		  </form>
