...
if(isset($_REQUEST['delid']) && $_REQUEST['delid']!='')
{
	...
	if($GETMessResqry)
	{
		header("Location:manage_Campaigns_list2.php?msg=deleted&page=".$_REQUEST['page']."&uid=".$_REQUEST['clientid']."");
		exit;
	}
...
<td align="right"  class="padbot5"><?php if($_REQUEST['uid']!="") {?> <a href="step1.php?ClientID=<?php echo $ClientID;?>">Add Campaign </a> <?php } else echo "&nbsp;"; ?></td>
  </tr>
  <tr class="grey-bg">
...
<table width="95%" cellpadding="0" cellspacing="1" id="customerGrid_table">
    <tr>
        <td height="30" align="left" class="tr1" width="15%"><b>Select Company </b></td>
		...
		<?php	while($RecGetClients=$object->FetchArray($ResGetClients)){?>
         	<option value="<?php echo $RecGetClients['clientid']?>" <?php if($_GET[uid]==$RecGetClients['clientid']) echo "selected='selected'"?>><?php echo $RecGetClients['company_name']?></option>
		<?php }?>
          </select>
		</td>
	</tr>
