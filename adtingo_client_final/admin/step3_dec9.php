...
function gettotalrecords(arealist,minage,maxage,gender,income,zip,radious,datesent,random_number)
{
	...
	for (var i=0; i < document.signupForm2.gender.length; i++)
   {
	 ...
	 req.open("GET","GetTotalMemberRecords_admin.php?minage="+minage+"&maxage="+maxage+"&gendersel="+rad_val+"&income="+income+"&area_list="+arealist+"&shddate="+datesent+"&random_number="+random_number,true);
	 req.send(null);
