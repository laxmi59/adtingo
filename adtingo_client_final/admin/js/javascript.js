...
function deleteCampaign_list(cid,page,clientid)
{
	var agree = confirm('Are you sure you want to delete this Campaign?');
	if(agree)
	  window.location.href="manage_Campaigns_list2.php?delid="+cid+"&page="+page+"&clientid="+clientid;

	
}
function deleteEmailTemplate(cid)
{
	var agree = confirm('Are you sure you want to delete this Email Template?');
	if(agree)
	  window.location.href="manage_email_templates.php?delid="+cid;
...
function setarticleasprimary(cat,nid,$page)
{
	var agree = confirm('Are you sure you want to Set this Article as Primary Artical in'+cat);
	if(agree)
	  window.location.href="category_article.php?art_id="+nid+"&page="+page;
...
function validateprofilefrm_admin()
{
	...
	if(document.profilrfrm.errormess.value=='1')
	{
...
function validatearticle()
{
	...
	var urlRegxp= /http:|\https:\/\/[A-Za-z0-9\.-]{3,}\.[A-Za-z]{3}/
	//regular expression which accets specified url extensions only
	
	if(trim(document.getElementById('cat_id').value)=='')
	{
		sErrStr +=" Select Category \n";
		if(sFieldName == "")
		sFieldName="cat_id";
	}
	...
	{
		if(urlRegxp.test(document.getElementById('url').value)==0)
		{
			sErrStr +=" Invalid URL \n";
			if(sFieldName == "")
			sFieldName="url";
	...
	if(trim(document.getElementById('main_image').value)!='')
	{
		if(imgexp.test(document.getElementById('main_image').value)==0)
		{
			sErrStr +=" Invalid Image \n";
			if(sFieldName == "")
			sFieldName="main_image";
		}
	}
...
function validateMetroArealist()
{
	...
	if(document.getElementById('cityerr').value==1)
	{
			sErrStr +=" City name already exists.  Please enter another City name \n";
			if(sFieldName == "")
			sFieldName="areaname";
	}
	if (sErrStr != "")
...
function member_citycheck(h,id)
{
	//alert(h);
	 var req=createRequest();
	 req.open("GET","emailverify.php?cityname="+h,true);
	 req.send(null);
	 req.onreadystatechange=function()
	 {
	 	//alert(req.readyState);alert(req.status);
