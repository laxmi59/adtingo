...
<br />
  ...
  <tr>
    ...
    <tr>
		...
		<?	while($RecGetClients=$object->FetchArray($ResGetClients)){?>
         	<option value="<?=$RecGetClients['clientid']?>" <? if($_GET[uid]==$RecGetClients['clientid']) echo "selected='selected'"?>><?=$RecGetClients['full_name']?></option>
		<? }?>
          </select>
		</td>
	</tr>
...
<br />
<? if($_GET[uid]){?>
<? $SqlGetCampaigns="SELECT * FROM `tbl_campaigns` WHERE `clientid` =$_GET[uid] AND `template_id` <>0 AND `mailing_ID` <>0 AND `status` =3";
...
<tr>
    <td align="left">page <?php echo $object->userpaginationBottom($cols,$limit,$page,"manage_reports.php?uid=$_GET[uid]"); ?> of <?php echo $let_infe;?> pages        |   view
		  ...
		  <select name="limit" class="input2" onchange="limtvaltracks(this.value,'manage_reports',<?=$_GET[uid]?>);">
...
<table width="100%" cellpadding="0" cellspacing="1" id="customerGrid_table">
            <thead>
              ...
              <!--<td width="6%" class="headings" align="left"><strong>ID</strong></td>-->
                <td width="17%" align="left" class="headings">Campaign Name <a href="manage_reports.php?uid=<?=$_GET[uid]?>&page=<?php echo $page;?>&field=campaign_name&order=ASC&limit=<?php echo $limit;?>"><img src="images/up.gif"  /></a><a href="manage_reports.php?uid=<?=$_GET[uid]?>&page=<?php echo $page;?>&field=campaign_name&order=DESC&limit=<?php echo $limit;?>"><img src="images/down.gif"  /></a></td>
                <td width="15%" align="left" class="headings">Subject <a href="manage_reports.php?uid=<?=$_GET[uid]?>&page=<?php echo $page;?>&field=subject&order=ASC&limit=<?php echo $limit;?>"><img src="images/up.gif"  /></a><a href="manage_reports.php?uid=<?=$_GET[uid]?>&page=<?php echo $page;?>&field=subject&order=DESC&limit=<?php echo $limit;?>"><img src="images/down.gif"  /></a></td>
		...
		<tr>
             <!--  <td class="<?php echo $class;?>" align="left"> </td>-->
                ...
                <td align="left" class="<?php echo $class;?>"> <?php echo stripslashes($SqlGetCampaignrec['schedule_date']);?></td>
                
                <td align="left" class="<?php echo $class;?>">
				<a href="campaign-performance-report.php?uid=<?=$_GET[uid]?>&rid=<?php echo stripslashes($SqlGetCampaignrec['campaign_id']);?>"><img src="images/reports.png" /></a>
		    </td>
              </tr>			  
			  <?php
			  $i++;
