...
?></strong></h4>
	...
	<?php /*?><h2 class="blue-tit m-top10"><span class="f-left">Search AdTingo</span><span class="view-archives f-right">VIEW ARCHIVES</span></h2><?php */?>
...
<a href="<?=home_url( '/' ).$name?>"><?php /*?><img alt="" width="130" height="130" src="<?php echo home_url( '/' ); ?>/wp-content/themes/adtingo/images/<?=$img?>" /><?php */?>
<img alt="" width="130" height="130" src="<?php echo home_url( '/' ); ?>client/Campaign_images/clickble_image/thumb_images/<?=$img?>" />
</a></dt>
...
<?="<h2 class='tit'><a href='".home_url( '/' ).$name."'>".stripslashes($title)."</a></h2><br>"?>
<?="<div>".substr(stripslashes($content), 0, 300)." <a href='".home_url( '/' ).$name."'class='read-more'> Read more... </a></div>"?>
</dd>
<? }?>
<?php
				...
				if($subcat==""){
					...
					while($term_tax_fet=mysql_fetch_array($term_tax)){
						...
						while($sel_rel_fet=mysql_fetch_array($sel_rel)){ 
							if($spost_fet['ID']<>''){
							$i=1;
								$des= disp($spost_cont['clickble_image'],$spost_fet['post_name'],$spost_fet['ID'],$spost_fet['post_date'],$spost_fet['post_title'],$spost_cont['text_content']);
								echo $des;
							}else{ continue;}
						}
					}
