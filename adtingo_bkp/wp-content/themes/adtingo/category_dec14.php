...
<? print_r(wp_get_post_categories('Atlanta'));?>
...
<?="<h2 class='tit'><a href='".home_url( '/'.$selcatterms['slug'].'/' ).$name."'>".stripslashes(substr($title,0,40))."</a></h2>"?>
<?="<div>".substr(stripslashes($content), 0, 300)."<br><span style='float:right'> <a href='".home_url( '/' ).$name."'class='read-more'> Read more... </a></span></div>"?>
</dd>
<? }?>
<?php
				...
				while($sel_rel_fet=mysql_fetch_array($sel_rel)){ 
					...
					$i=1;
						...
						if($spost_cont['clickble_image']<>""){
							$dispimg="client/Campaign_images/clickble_image/thumb_images/".$spost_cont['clickble_image'];		}elseif($spost_cont['main_image']=="" && $spost_cont['clickble_image']==""){
							$dispimg="client/images/no_pic130.gif";
						}
						$des= disp($dispimg,$spost_fet['post_name'],$spost_fet['ID'],$spost_fet['post_date'],$spost_fet['post_title'],$spost_cont['text_content']);
						echo $des;
				...
				while($terms_relation=mysql_fetch_array($terms_SubCat_Relation)){
					...
					$i=1;
						if($combfet['main_image']<>""){ 
							...
							$dispimg="client/Campaign_images/clickble_image/thumb_images/".$combfet['clickble_image'];						}elseif($combfet['main_image']=="" && $combfet['clickble_image']==""){
							$dispimg="client/images/no_pic130.gif";
						}
						$des= disp($dispimg,$spost['post_name'],$spost['ID'],$spost['post_date'],$spost['post_title'],$combfet['text_content']);
						echo $des;
