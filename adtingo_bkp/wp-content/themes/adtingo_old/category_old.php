...
<div class="content-left">
	...
	if($file=="compaign" || $file=="category")$catid=DEFAULT_CITY; else $catid=$file;
				...
				if($subcat==""){
					...
					while($term_tax_fet=mysql_fetch_array($term_tax)){
						...
						while($sel_rel_fet=mysql_fetch_array($sel_rel)){ $i=1;
							$des= disp($spost_fet['clickble_image'],$spost_fet['post_name'],$spost_fet['ID'],$spost_fet['post_date'],$spost_fet['post_title'],$spost_cont['text_content']);
							echo $des;
						}
					}
				}else{
		...
		if($terms_SubCat['parent']==0){
			...
			while($terms_SubCatp_fet=mysql_fetch_array($terms_SubCatp)){
				...
				while($terms_relation=mysql_fetch_array($terms_SubCat_Relation)){
					...
					$des= disp($combfet['clickble_image'],$spost['post_name'],$spost['ID'],$spost['post_date'],$spost['post_title'],$combfet['text_content']);
					echo $des;
				}
			}
		}else{
...
<? $i=0;
	...
	if($term_slug['slug']==$file){ $i=1;?>
		<? $des= disp($comb_fet['clickble_image'],$comb_fet['post_name'],$comb_fet['ID'],$comb_fet['post_date'],$comb_fet['post_title'],$comb_fet['text_content']);
		echo $des;?>
	<? }else{ 
		if($term_slug['parent']<>''){
...
?>
	<a href="<?php echo home_url( '/' ).$hot_row['post_name']?>"><img src="<?php echo home_url( '/' )?>wp-content/themes/adtingo/images/<?=$img_hot?>" /></a>
	<h3><a href="<?php echo home_url( '/' ).$hot_row['post_name']?>"><?=$hot_row['post_title']?></a></h3>
	<p><?=substr($hot_row_fet['text_content'],0,50)?></p>
	<p><a href="<?php echo home_url( '/' ).$hot_row['post_name']; ?>" class="read-more">Read More</a></p>
<? }else{
	echo "<h3 style='text-transform:capitalize;' align='center'>There is no hot topic today</h3>";
	}
?>
    </div>
  </div>
...
?>
	<dt><a href="<?php echo home_url( '/' ).$pop_row['post_name']?>"><img src="<?php echo home_url( '/' ); ?>wp-content/themes/adtingo/images/<?=$img?>" width="50" height="50" /></a></dt>
 	<dd><h2 class="tit"><a href="<?php echo home_url( '/' ).$pop_row['post_name'] ?>" title="<?php echo $pop_row['post_title'] ?>"><?php echo $pop_row['post_title'] ?></a></h2><?=substr($pop_row_fet['text_content'],0,30)?> <a href="<?php echo home_url( '/' ).$pop_row['post_name']?>" class="read-more">Read More</a></dd>
  <? }?>   
 <? }else{?>
