...
if($pop_row_fet['main_image']<>""){ 
	...
	<dt><a href="<?php echo home_url( '/' ).$pop_row['post_name']?>"><img src="<?php echo home_url( '/' ); ?><?=$dispimg?>" width="60" height="60" /></a></dt>
 	<dd><h2 class="tit"><a href="<?php echo home_url( '/' ).$pop_row['post_name'] ?>" title="<?php echo $pop_row['post_title'] ?>"><?php echo stripslashes(stripslashes($pop_row['post_title'])) ?></a></h2>
	<?=substr(stripslashes(stripslashes($pop_row_fet['text_content'])),0,30)?><br/> <a href="<?php echo home_url( '/' ).$pop_row['post_name']?>" class="read-more">Read More</a></dd>
  <? }?>   
