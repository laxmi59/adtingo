		...
		if(!$listing_id_error){
			if($Num_cols >0)
			{
				...
				if($_POST["templatename"]==3 || $_POST["templatename"]==4){
					...
					//echo $arrXml['location']['name'];	exit;
					
					$Insert_campaign_info_qry=sprintf("update tbl_campaigns  set template_selection=%d, heading='%s', sub_heading='%s', text_content='%s', map_address='%s', map_city='%s', map_state ='%s', phone =%d, websiteurl ='%s', map_zipcode ='%s', destination_url ='%s', listing_id=%d where campaign_id=%d",$object->stripper($_POST["templatename"]), $object->stripper(addslashes($arrXml['location']['name'])), $object->stripper(addslashes($arrXml['location']['teaser'])), $object->stripper(addslashes($arrXml['location']['customer_content']['customer_message'])),$object->stripper(addslashes($arrXml['location']['address']['street'])), $object->stripper(addslashes($arrXml['location']['address']['city'])), $object->stripper(addslashes($arrXml['location']['address']['state'])), $object->stripper($arrXml['location']['contact_info']['display_phone']), $object->stripper(addslashes($arrXml['location']['contact_info']['display_url'])), $object->stripper($arrXml['location']['address']['postal_code']), $object->stripper(addslashes($arrXml['location']['urls']['profile_url'])),$object->stripper($_POST['listing_id']), $object->stripper($CampaignID));
