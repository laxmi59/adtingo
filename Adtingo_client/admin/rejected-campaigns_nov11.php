...
if(isset($_REQUEST['delid']) && $_REQUEST['delid']!='')
{
	...
	if($GETMessResqry)
	{
		header("Location:rejected-campaigns.php?msg=del&page=".$_REQUEST['page']."");
		exit;
	}
...
< $miles order by distance";
 ...
 $totalemailsids=$object->GetCampaignSearchResult($RecCampaign_Seg_list_info['area_list'],$RecCampaign_Seg_list_info['gender'],$RecCampaign_Seg_list_info['education'],$RecCampaign_Seg_list_info['income'],$RecCampaign_Seg_list_info['keywords'],$RecCampaign_Seg_list_info['minimum_age'],$RecCampaign_Seg_list_info['maxmum_age'],$zipcodedb);
			...
			header("Location:rejected-campaigns.php?msg=$msg&page=".$_REQUEST['page']);
			exit;
	}
...
function deleteCampaign(cid,page)
{
	...
	if(agree)
	  window.location.href="rejected-campaigns.php?delid="+cid+"&page="+page;
...
per page</td>
  ...
  <tr>
	...
	<?php if($SqlGetCampaignrec['status']=='5')
	  {
	  ?>	
		<a  href="rejected-campaigns.php?cid=<?php echo $SqlGetCampaignrec['campaign_id'];?>&status=2&page=<?php echo $page ;?>" onclick="javascript:return confirm('Are you sure you want to Approve this campaign');" >
		<strong>Approve</strong></a>&nbsp;&nbsp; | &nbsp;&nbsp;
		
		<a  href="rejected-campaigns.php?cid=<?php echo $SqlGetCampaignrec['campaign_id'];?>&status=4&page=<?php echo $page ;?>" onclick="javascript:return confirm('Are you sure you want to Reject this campaign');" >
