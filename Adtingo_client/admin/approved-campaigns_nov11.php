...
if(isset($_REQUEST['delid']) && $_REQUEST['delid']!='')
{
	...
	if($GETMessResqry)
	{
		header("Location:approved-campaigns.php?msg=del&page=".$_REQUEST['page']."");
		exit;
	}
...
< $miles order by distance";
 ...
 $totalemailsids=$object->GetCampaignSearchResult($RecCampaign_Seg_list_info['area_list'],$RecCampaign_Seg_list_info['gender'],$RecCampaign_Seg_list_info['education'],$RecCampaign_Seg_list_info['income'],$RecCampaign_Seg_list_info['keywords'],$RecCampaign_Seg_list_info['minimum_age'],$RecCampaign_Seg_list_info['maxmum_age'],$zipcodedb);
			...
			header("Location:approved-campaigns.php?msg=$msg&page=".$_REQUEST['page']);
			exit;
	}
...
//********** Start Code for displaying CUSTOMERS  *************//
	 ...
	 $SqlGetCampaigns="select *,date_format(schedule_date,'%d %b,  %Y') as date ,date_format(date_created ,'%W %b %d,  %Y') as createddate  from tbl_campaigns where status ='2' OR status ='3'";
	...
	{
		 $SqlGetCampaigns.="  Order By schedule_date ASC";
	}
...
function deleteCampaign(cid,page)
{
	...
	if(agree)
	  window.location.href="approved-campaigns.php?delid="+cid+"&page="+page;
...
<table width="95%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td align="left" colspan="3" class="content-header"><a href="manage_campaign.php">Pending Campaigns</a> <a href="approved-campaigns.php" class="select">Approved Campaigns</a><a href="rejected-campaigns.php">Rejected Campaigns</a></td>
...
<tr>
   ...
   <tr>
    <td align="left">page <?php echo $object->userpaginationBottom($cols,$limit,$page,"approved-campaigns.php?"); ?> of <?php echo $let_infe;?> pages        |   view
	
		  <select name="limit" class="input2" onchange="limtval(this.value,'approved-campaigns');">
...
per page</td>
  ...
  <tr>
          ...
          <td class="form-bg2"><table width="100%" cellpadding="0" cellspacing="1" id="customerGrid_table">
            <thead>
			  ...
			  <td width="15%" class="headings" align="left"><strong>Company Name</strong></td>
                <td width="17%" align="left" class="headings">Campaign Name <a href="approved-campaigns.php?page=<?php echo $page;?>&field=campaign_name&order=ASC&limit=<?php echo $limit;?>"><img src="images/up.gif"  /></a><a href="approved-campaigns.php?page=<?php echo $page;?>&field=campaign_name&order=DESC&limit=<?php echo $limit;?>"><img src="images/down.gif"  /></a></td>
                <td width="15%" align="left" class="headings">Subject <a href="approved-campaigns.php?page=<?php echo $page;?>&field=subject&order=ASC&limit=<?php echo $limit;?>"><img src="images/up.gif"  /></a><a href="approved-campaigns.php?page=<?php echo $page;?>&field=subject&order=DESC&limit=<?php echo $limit;?>"><img src="images/down.gif"  /></a></td>
	...
	<?php if($SqlGetCampaignrec['status']=='5')
	  {
	  ?>	
		<a  href="approved-campaigns.php?cid=<?php echo $SqlGetCampaignrec['campaign_id'];?>&status=2&page=<?php echo $page ;?>" onclick="javascript:return confirm('Are you sure you want to Approve this campaign');" >
		<strong>Approve</strong></a>&nbsp;&nbsp; | &nbsp;&nbsp;
		
		<a  href="approved-campaigns.php?cid=<?php echo $SqlGetCampaignrec['campaign_id'];?>&status=4&page=<?php echo $page ;?>" onclick="javascript:return confirm('Are you sure you want to Reject this campaign');" >
