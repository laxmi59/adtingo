...
if($_REQUEST['client_view_status']!='')
{
	if($_REQUEST['client_view_status']=='on')
		$msg='ClientAct';
	else
		$msg='ClientDeact'; 
	
	$QryupdateUserStatus=sprintf("update tbl_email_template_content set client_view_status='%s' Where TId	=%d",$object->stripper($_REQUEST['client_view_status']),$object->stripper($_REQUEST['eid']));
	$ResupdateUserStatus=$object->ExecuteQuery($QryupdateUserStatus);
	if($ResupdateUserStatus)
	{
		header("Location:manage_email_templates.php?msg=$msg&page=".$_REQUEST['page']);
...
if($_REQUEST['msg']=="ClientAct")
{
?>
 Email Template has been activated for Clients view successfully
<?php
}
if($_REQUEST['msg']=="ClientDeact")
{
?>
 Email Template has been deactivated for Clients view successfully
<?php
}
?>  
    </td>
...
<table width="95%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<img src="images/preview.png" alt="Preview" title="Preview" /></a>
	...
	<?php if($RecGetEmailTemplates['client_view_status']=='on'){ $st='off'; $stt="Deactivate";}else{$st='on'; $stt="Activate";}?>
	<a href="manage_email_templates.php?eid=<?php echo $RecGetEmailTemplates['TId'];?>&client_view_status=<?php echo $st;?>&page=<?php echo $page ;?>" onclick="javascript:return confirm('Are you sure you want to <?php echo $stt;?> this Email Template for Clients View');" ><input type="checkbox" name="client_view_status" <?php if($RecGetEmailTemplates['client_view_status']=='on'){?> checked="checked"<?php }?> alt="Client View" title="Client View" /></a></span>
