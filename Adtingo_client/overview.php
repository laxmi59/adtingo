...
if(isset($_REQUEST['delid']) && $_REQUEST['delid']!='')
{
 ...
 $SqlCampaign_info="select *,date_format(schedule_date,'%d %b,  %Y') as date ,date_format(date_created ,'%W %b %d,  %Y') as createddate  from tbl_campaigns where (status=1 OR status=5) and clientid=".$Clientid ; 
...
if(!isset($_REQUEST['page']))
	...
	$let_infe=ceil($cols/$limit);
	 $SqlCampaign_info.=" order by schedule_date asc"; 
...
<!--body-->
  ...
  <?php if($_REQUEST['msg']=="del"){ echo "Campaign has been deleted successfully"; } ?>
    ...
    <table class="tableHeader"  width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td >
		  <?php if($RecCampaign_info['status']==1) {?>
		  ...
		  <a href="edit-campaign.php?cid=<?php echo base64_encode($RecCampaign_info['campaign_id']);?>"><img src="images/edit.png" alt="Edit Campaign"  title="Edit Campaign"/></a>
		  <?php } ?>
		   <?php if($RecCampaign_info['status']==3 ){?>
		   <?php 
			$getreport=charts($RecCampaign_info['mailing_ID']);
			$deliveredCount=$getreport->deliveredCount;
			if($deliveredCount<>0){?>
		   <a href="campaign-performance-report.php?rid=<?php echo $RecCampaign_info['campaign_id'];?>"><img src="images/reports.png"  alt="Campaign Report" title="Campaign Report"/></a>
		   <?php }else{?>
		   <img src="images/rep_not_avail.jpg" width="24" height="24" alt="Report Not Available" title="Report Not Available"/>
		   <?php }}?>
		  
		   <a href="javascript:deleteCampaign(<?php echo $RecCampaign_info['campaign_id'];?>,<?php echo $page;?>);" ><img src="images/delete.png" title="Delete Campaign" alt="Delete Campaign" /></a></td>
        </tr>
		<?php
		 } 
		?>
