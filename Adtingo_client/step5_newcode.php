...
if($_REQUEST['cid']!="")
{
	$CampaignID=base64_decode($_REQUEST['cid']);
  	 $SqlCampaign_info="select *,date_format(schedule_date,'%d %b,  %Y') as ssdate from tbl_campaigns where campaign_id=".$CampaignID ; 
                ...
                <div class="small-form"> 
				  ...
				  <input type="text" name="schedule_date" id="schedule_date"
				   ...
				   $dateformat1=explode("-",$dateformat[0]);
					...
					<?php
					}else { echo "<strong>".$RecCampaign_info['ssdate']."</strong>";
					$dateformat=explode(" ",$RecCampaign_info['schedule_date']);
				   $dateformat1=explode("-",$dateformat[0]);
				     $dateformat2=$dateformat1['1']."/".$dateformat1['2']."/".$dateformat1['0']; ?>
					<input type="hidden" name="schedule_date" id="schedule_date" value="<?php echo $dateformat2;?>" />
					<?php }?>
