...
/*Display Categories */
function wp_show_categories( $iid ) {
//echo $iid;
	//$Sel_Sub_Cat=mysql_query("SELECT * FROM `wp_term_taxonomy` as tab1, `wp_terms` as tab2 where tab1.taxonomy = 'category' and tab1.parent = $iid and tab1.term_id = tab2.term_id ");
	$ssl=mysql_fetch_array(mysql_query("select * from `wp_terms` where slug='$iid'"));
	$Sel_Sub_Cat=mysql_query("SELECT * FROM `wp_term_taxonomy` as tab1, `wp_terms` as tab2 where tab1.taxonomy = 'category' and tab1.parent = '$ssl[term_id]' and tab1.term_id = tab2.term_id order by tab1.term_id asc ");
	$Sel_Sub_Cat_Num = mysql_num_rows($Sel_Sub_Cat);?> 
<div class="menu-adtingo">
	<ul class="top-menunav"><?
	$ex2=$_SERVER['REQUEST_URI'];
	$file2 = basename(rtrim($ex2, '/'));
	if($Sel_Sub_Cat_Num){	
		while($Sel_Sub_Cat_Fet = mysql_fetch_array($Sel_Sub_Cat)){
		if($file2==$Sel_Sub_Cat_Fet['slug']){ ?>
		<li id="active">
		<? }else{?>
		<li><? }?>
			<? if($Sel_Sub_Cat_Fet['parent']<>0){?>
			<a href="<?php echo home_url( '/' ); ?><?=$iid?>/<?=$Sel_Sub_Cat_Fet['slug'];?>"><?=$Sel_Sub_Cat_Fet['name'];?></a>
			<? }else{?>
			<a href="<?=$iid?>/<?=$Sel_Sub_Cat_Fet['slug'];?>"><?=$Sel_Sub_Cat_Fet['name'];?></a>
			<? }?>
		</li>
		<?	}
	}?>
	...
	<? }
function wp_show_categories_footer( $iid ) {
	$ssl=mysql_fetch_array(mysql_query("select * from `wp_terms` where slug='$iid'"));
	$Sel_Sub_Cat=mysql_query("SELECT * FROM `wp_term_taxonomy` as tab1, `wp_terms` as tab2 where tab1.taxonomy = 'category' and tab1.parent = '$ssl[term_id]' and tab1.term_id = tab2.term_id order by tab1.term_id asc ");
	$Sel_Sub_Cat_Num = mysql_num_rows($Sel_Sub_Cat);
	if($Sel_Sub_Cat_Num){	
		while($Sel_Sub_Cat_Fet = mysql_fetch_array($Sel_Sub_Cat)){?>
			...
			<? if($Sel_Sub_Cat_Fet['parent']<>0){?>
			<a href="<?php echo home_url( '/' ); ?><?=$iid?>/<?=$Sel_Sub_Cat_Fet['slug'];?>"><?=$Sel_Sub_Cat_Fet['name'];?></a><? if($Sel_Sub_Cat_Fet['name']=='Dating'){echo "<br>";}?>&nbsp;|&nbsp;
			<? }else{?>
			<a href="<?=$iid?>/<?=$Sel_Sub_Cat_Fet['slug'];?>"><?=$Sel_Sub_Cat_Fet['name'];?></a><? if($Sel_Sub_Cat_Fet['name']=='Dating'){echo "<br>";}?>&nbsp;|&nbsp;
			<? }?>
