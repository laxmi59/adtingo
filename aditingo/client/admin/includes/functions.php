...
//***** START CODE FOR RETRVING SELECTED SUBSCRIPTION DETAILS *******//

function getSubscritpionInfo($subid)
{
	
	$getsubtype=sprintf("select * from tbl_subscriptions where SubscriptionID=%d ",$this->stripper($subid));
	$Qrysubtype=$this->ExecuteQuery($getsubtype);	
	$Ressubtype=$this->FetchArray($Qrysubtype);
	$subdetails=$Ressubtype['TracksNum'].'*'.$Ressubtype['Price'];
	return $subdetails;
}
//***** START CODE FOR RETRVING SELECTED SUBSCRIPTION SETAILS *******//

//***** START CODE FOR RETRVING SELECTED Primary Genere NAME *******//
function GetUserEmail($UserID)
{
	$SqlGetUserEmail=sprintf("select Email FROM tbl_users where UserID=%d ",$this->stripper($UserID));
	$QryGetUserEmail=$this->ExecuteQuery($SqlGetUserEmail);
	$ResGetUserEmail=$this->FetchArray($QryGetUserEmail);
	$Email=$ResGetUserEmail['Email'];
...
function thumbImage($source,$dest,$new_width,$new_height,
