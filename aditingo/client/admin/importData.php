...
if(isset($_POST['button3']) && $_POST['button3']!='')
{
	...
	if ($row > 0) {
		...
		if($email!='' && $username!='')
		{
		  ...
		  $SqlGetInfo="select * from tbl_members  where email_address='".$email."' OR username='".$username."'";//exit;
		  $QryGetInfo=$object->ExecuteQuery($SqlGetInfo);
		   $numrows=mysql_num_rows($QryGetInfo);
			...
			$new_rand_pwd=$rand_numbers.$rand_alpha;			 
			 $insertMember=sprintf("insert into tbl_members(full_name,last_name,email_address,username,password,home_city,dob,gender,zipcode,education,income,  	interests_and_activities,contact_time,date_created,status)
		values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')",$name,$lastname,$email,$username,$new_rand_pwd,$cityname,$dob,$gender,$zipcode,$educationkey,$incomekey,$interestkey,'1',$current_date,'1'); 				$resinsertmember=$object->ExecuteQuery($insertMember);
...
<tr><td colspan="3" align="center"><font style='font-family:Arial;font-weight:bold;color:#336600; font-size:13px;'><?php if($mess!="") { echo "Data has been imported successfully"; }?></font></td></tr>
                </table>
