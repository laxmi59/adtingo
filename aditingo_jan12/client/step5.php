...
//$TotalRecords=$object->GetTotalNumRecords($_SESSION['arealist']);
	...
	if($_POST['sendimmediately']=='1' || $_POST['sendimmediately']=='2')
	{
		$_SESSION['campaign']['sendimmediately']=$_POST['sendimmediately'];
		if($_POST['sendimmediately']=='1')
		{
			$count=$count+1;
		}
		else
		{
			...
			{
			$_SESSION['campaign']['schedule_date']=$_POST['schedule_date'];
			...
			else
			{
				if($_POST["schedule_date"]=="")
				{
				$sending_options_error="Date Field Required";
				$_SESSION['campaign']['schedule_date']="";
				}
				else
				$_SESSION['campaign']['schedule_date']=$_POST["schedule_date"];
		...
		{
		$Schedule_Message_error="Select The Schedule Message option";
		}
		...
		if($count>0)
			{
				...
				$date_format2=$date_format1['2']."-".$date_format1['0']."-".$date_format1['1'];
			 	...
			 	$Insert_seg__List_qry=sprintf("update tbl_campaigns  set sending_option =%d,schedule_date ='%s' where campaign_id=%d",$object->stripper($_POST["sendimmediately"]),$object->stripper($schedule_date),$object->stripper($CampaignID)); 
      ...
      <form  action="" method="post" name="signupForm5" id="signupForm5">                              
            ...
            <h3 class="grad-box">Schedule Message</h3>            
                <div class="small-form"> 
               	 <input  type="radio" value="1" name="sendimmediately" id="sendimmediately" onclick="return Campaign_Sending_options();" <?php if(($_SESSION['campaign']['sendimmediately']=='1') || ($RecCampaign_info['sending_option']=='1')) { ?> checked="checked" <?php } ?> /> Send Immediately After Approval<br />
                 <input type="radio" value="2" name="sendimmediately" id="schedule_date_time" onclick="return Campaign_Sending_options();" <?php if(($_SESSION['campaign']['sendimmediately']=='2') ||$RecCampaign_info['sending_option']=='2') { ?> checked="checked" <?php } ?>/> Date  
                  <input type="text" name="schedule_date" id="schedule_date"
				   value="<?php if($RecCampaign_info['sending_option']=='2')
				   {
				   $dateformat=explode(" ",$RecCampaign_info['schedule_date']);
				   $dateformat1=explode("-",$dateformat[0]);
				   $Timeformat1=explode("-",$dateformat[1]);
