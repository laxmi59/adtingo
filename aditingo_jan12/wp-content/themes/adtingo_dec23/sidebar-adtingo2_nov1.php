...
<? $ex1=$_SERVER['REQUEST_URI'];
	$file = basename(rtrim($ex1, '/'));
		...
		if($ssl<>0){
			...
			$sel=mysql_fetch_array(mysql_query("select * from `tbl_catart` where cat_id=$sst[term_id] and `primary_cat`='on'"));
			if($sel)$err=1;
		}elseif($ss2<>0){
			$ss2=mysql_fetch_array(mysql_query("select * from `wp_terms` where `slug`='".$file."'"));
			$sel=mysql_fetch_array(mysql_query("select * from `tbl_catart` where cat_id=$ss2[term_id] and `primary_cat`='on'"));
			if($sel)$err=1;
		}else{
