<div class="category-cont">
	<? $hot_row=mysql_fetch_array(mysql_query("select * from `wp_postmeta` as tab1, `wp_posts` as tab2 where tab1.`meta_key` = 'hot topic' and tab1.`meta_value` = 'checkbox_on' and tab1.post_id=tab2.ID order by tab2.ID desc limit 0,1 "));
if($hot_row){
	$hot_row_fet=mysql_fetch_array(mysql_query("select * from `campaign_posts` as tab3, `tbl_campaigns` as tab4 where  tab3.post_id = '$hot_row[ID]' and tab3.campaign_id= tab4.campaign_id"));
	
	$categories = get_the_category($hot_row[ID]);
	$maincat=mysql_fetch_array(mysql_query("select * from wp_terms where term_id=".$categories[0]->category_parent));
?>
<?php
if($hot_row_fet['main_image']<>""){ 
	$dispimg="client/Campaign_images/main_image/thumb_280x140/".$hot_row_fet['main_image'];
}elseif($hot_row_fet['clickble_image']<>""){
	$dispimg="client/Campaign_images/clickble_image/thumb_images/".$hot_row_fet['clickble_image'];
}elseif($hot_row_fet['main_image']=="" && $hot_row_fet['clickble_image']==""){
	$dispimg="client/images/no_pic130.gif";
}?>
	<a href="<?php echo home_url( '/'.$maincat['slug'].'/'.$categories[0]->slug.'/' ).$hot_row['post_name']?>"><img src="<?php echo home_url( '/' ); ?><?=$dispimg?>" width="280" height="140" /></a>
	<h3><a href="<?php echo home_url( '/'.$maincat['slug'].'/'.$categories[0]->slug.'/' ).$hot_row['post_name']?>"><?=stripslashes($hot_row['post_title'])?></a></h3>
	<p><?=substr(stripslashes($hot_row_fet['text_content']),0,50)?></p>
	<p><a href="<?php echo home_url( '/'.$maincat['slug'].'/'.$categories[0]->slug.'/' ).$hot_row['post_name']; ?>" class="read-more">Read More</a></p>
<? }else{
	echo "<div><h3 style='text-transform:capitalize; height:100px' align='center'>There is no hot topic today</h3></div>";
	echo "<p>&nbsp;</p>";
	}
?>
</div>


    