...
if($_POST['but']!='')
{
	...
	if($_POST['email']!='' && $_POST['metroIds']!='')
	{
		...
		else
		{
			...
			$insert=mysql_query("insert into `tbl_members` (`email_address`, `password`, `zipcode`, `date_created`, `status`, `contact_time`) values ('".$_POST[email]."', '".$string."', '".$_POST[zipcode]."', '$date','1','1')");
			if($insert){
    ...
    <form onsubmit="return validateformStep1();" method="post" name="registration" action="<?php echo home_url(); ?>/?page_id=19">
      <div id="div-Form">
        ...
        <div class="register-form">
          <div align="center" class="errer-mess2"><?php echo $error_mess;?></div>
          <dl class="form1">
