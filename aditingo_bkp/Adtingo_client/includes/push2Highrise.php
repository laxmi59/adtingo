...
class Push_Highrise{
	...
	var $highrise_url = 'https://adtingo.highrisehq.com'; // your highrise url, e.g. http://yourcompany.highrisehq.com
	var $api_token = '1779202b93e2d0cf3e781d0fca61e0fc'; // your highrise api token; can be found under My Info
	var $task_assignee_user_id = 'prasad.ritwik'; // user id of the highrise user who gets the task assigned 
	...
	function pushContact($request){
		...
		if($id < 0){
			...
			curl_setopt($curl,CURLOPT_POST,true);
				...
				<first-name>'.htmlspecialchars($request['full_name']).'</first-name>
				<last-name>'.htmlspecialchars($request['sLastName']).'</last-name>
				<background>'.htmlspecialchars($request['staff_comment']).'</background>
				<company-name>'.htmlspecialchars($request['companyname']).'</company-name>
				<contact-data>
					<email-addresses>
						<email-address>
	...
	function _person_in_highrise($person){
		$curl = curl_init($this->highrise_url.'/people/search.xml?term='.urlencode($person['full_name']));
		curl_setopt($curl,CURLOPT_RETURNTRANSFER,true);
